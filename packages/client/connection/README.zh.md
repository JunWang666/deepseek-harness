...
认证之前，每个请求仍经过 `src/api-request-trust.ts`。其 `Host` 必须是 loopback，或与 `trustedHosts` 条目匹配：带端口的 `host:port` 精确匹配，不带端口的条目匹配任意端口，两侧均经 WHATWG 归一化。若附带 `Origin`，它必须等于该 Host；`sec-fetch-site: cross-site` 一律拒绝。畸形配置 authority 会让插件加载失败。这些检查防御 DNS rebinding 与跨站浏览器请求，绝不建立身份。Host/Origin 校验失败返回 403；Host 可信但未认证的请求返回 401。`dsh web --host 0.0.0.0` 要求显式启用远程访问。决策记录：[浏览器请求信任](../../../.agents/notes/implemented/architecture/2026-07-28-api-browser-trust-boundary.zh.md)与[浏览器令牌认证](../../../.agents/notes/implemented/architecture/2026-08-24-browser-token-authentication.zh.md)。

`unsafeAllowRemote` 默认为 `false`。设置为 `true` 后，所有 HTTP API 路由和 WebSocket 升级均跳过请求信任与浏览器认证，首页请求无需凭据，启动 URL 不携带令牌。Host 将该策略注入所提供的页面，让远程浏览器也能使用通常仅限 loopback 页面的管理功能。[Web bundle](../../bundle/web-app/README.zh.md#lan-access-and-trusted-hosts) 将 `DSH_UNSAFE_ALLOW_REMOTE=1` 映射到此字段。这会向所有能连接的调用者授予完整 Host 管理权限；JSON 校验和请求体大小限制仍然强制执行。
