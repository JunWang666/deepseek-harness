...
Before authentication, every request still passes `src/api-request-trust.ts`. Its `Host` must be loopback or match a `trustedHosts` entry: exact on `host:port`, any port on port-less entries, both sides WHATWG-normalized. An attached `Origin` must equal that Host and `sec-fetch-site: cross-site` is refused. Malformed configured authorities fail plugin load. These checks defend DNS rebinding and cross-site browser requests; they never establish identity. A failed Host/Origin check returns 403, while a trusted but unauthenticated request returns 401. `dsh web --host 0.0.0.0` requires the explicit remote-access opt-in. Decision records: [browser request trust](../../../.agents/notes/implemented/architecture/2026-07-28-api-browser-trust-boundary.md) and [browser token authentication](../../../.agents/notes/implemented/architecture/2026-08-24-browser-token-authentication.md).

`unsafeAllowRemote` defaults to `false`. Setting it to `true` bypasses request trust and browser authentication for all HTTP API routes and WebSocket upgrades, permits index requests without credentials, and leaves launch URLs token-free. The Host injects this policy into served pages so remote browsers can use management features normally limited to loopback pages. The [Web bundle](../../bundle/web-app/README.md#lan-access-and-trusted-hosts) maps `DSH_UNSAFE_ALLOW_REMOTE=1` to this field. This grants complete Host administration to every reachable caller; JSON validation and request-body limits remain enforced.
