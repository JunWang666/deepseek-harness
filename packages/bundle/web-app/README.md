...
Run `dsh --profile web` to open an interactive browser GUI with chat, model and settings management, and session history. It uses the same model access, tools, and safety defaults as other dsh surfaces. Startup prints an authenticated URL and normally opens it in the default browser; SSH sessions and `--no-open` leave the URL for manual opening. You can change the port and enable unrestricted remote administration explicitly. Choose this package for interactive browser work; use `dsh-headless` for one-shot command-line tasks.
...
By default the GUI accepts connections from this machine only. A deployment that binds all network interfaces also allows browsers from the LAN, and the printed URL then includes a LAN address; `--trusted-host` adds extra hosts in either case. Under the default policy, Host and Origin checks control reachability, while the token exchange authenticates every Host API method and WebSocket stream. The LAN addresses are sampled once at startup, so a network change later is not picked up — restart the GUI to re-advertise.

Set `DSH_UNSAFE_ALLOW_REMOTE=1` before starting `dsh --profile web` to accept unrestricted remote administration. The shipped profile defaults to `0.0.0.0` in this mode; an explicit `--host 127.0.0.1` keeps the listener local. Connection skips Host, Origin, Fetch-Metadata, and browser-cookie checks for HTTP API calls and WebSocket upgrades, serves the index without a token, and enables management features on remote pages. Anyone who can reach the port can control the Host, including settings, credentials, and tool execution. Only the exact value `1` enables this mode; unset, empty, `0`, and `true` retain the default policy. Restart after changing the variable. Request schemas, body limits, and tool permission rules still apply.
...
- **Remote administration has no built-in access control when opted in** — `DSH_UNSAFE_ALLOW_REMOTE=1` makes all reachable callers administrators.
